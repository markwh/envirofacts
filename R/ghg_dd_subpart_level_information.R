#' Retrieve dd subpart level information data from ghg database
#' 
...
#' @param GHG_NAME e.g. 'Sulfur hexafluoride'. See Details.
#' @param GHG_QUANTITY e.g. '0'. See Details.
#' @export

ghg_dd_subpart_level_information <- function(FACILITY_ID = NULL, REPORTING_YEAR = NULL, 
    FACILITY_NAME = NULL, GHG_NAME = NULL, GHG_QUANTITY = NULL) {
    
    args <- list(FACILITY_ID = FACILITY_ID, REPORTING_YEAR = REPORTING_YEAR, FACILITY_NAME = FACILITY_NAME, 
        GHG_NAME = GHG_NAME, GHG_QUANTITY = GHG_QUANTITY)
    ret <- envir_get(dd_subpart_level_information, args)
